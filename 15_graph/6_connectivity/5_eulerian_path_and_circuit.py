...
# Vertices with zero degree are not considered since they don't belong to Eulerian path
# (2) Zero or two vertices have odd degree, and all others have even degree
# Only one vertex with odd degree is not possible in an undirected graph
# Sum of all degrees is always even in an undirected graph

# An undirected graph has eulerian cycle if these two conditions are true
# (1) Same as that of eulerian path
# (2) All vertices have even degree

# A graph is called Eulerian if it has an Eulerian cycle
# And Semi-Eulerian if it has an Eulerian path
# A graph with no edges is considered Eulerian since there are no edges to traverse
